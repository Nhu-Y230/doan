// ============================================================
//  firebase.js — 3 CON BÁO CINEMA
//  Gộp: Firebase config + Auth/Firestore + java.js + Tracker
// ============================================================
...
// ════════════════════════════════════════════════════════════
//  TRACKER — Ghi lịch sử trang đã ghé thăm
//  Tự động chạy khi tải trang, không cần gọi thêm gì cả.
// ════════════════════════════════════════════════════════════

// Tên trang hiện tại (dùng title + tên file)
...
// Tạo hoặc lấy ID khách từ localStorage (dùng cho khách chưa đăng nhập)
function layHoacTaoIDKhach() {
  let id = localStorage.getItem("khach_id");
  if (!id) {
    id = "khach_" + Date.now() + "_" + Math.random().toString(36).slice(2, 8);
    localStorage.setItem("khach_id", id);
  }
  return id;
}

// Ghi vào Firestore collection "lich_su_trang"
async function ghiLichSuTrang(docId, emailHienThi) {
  try {
    const ref = doc(db, "lich_su_trang", docId);
    await setDoc(ref, {
      email_khach:   emailHienThi,
      cap_nhat_cuoi: serverTimestamp(),
      trang_da_ghe:  arrayUnion({
        trang:     layTenTrang(),
        thoi_gian: new Date().toLocaleString("vi-VN")
      })
    }, { merge: true });
  } catch (err) {
    console.warn("[Tracker] Lỗi ghi Firestore:", err);
  }
}
...
onAuthStateChanged(auth, (user) => {
  if (user) {
    // Đã đăng nhập → dùng UID + email thật
    ghiLichSuTrang(user.uid, user.email);
  } else {
    // Khách chưa đăng nhập → dùng ID tự sinh trong localStorage
    const khachId = layHoacTaoIDKhach();
    ghiLichSuTrang(khachId, "khách-chưa đăng nhập");
  }
});
...
async function handleLogin(e) {
  ...
  try {
    ...
    saveCurrentUser({ name: user.displayName || email.split('@')[0], email: user.email, avatar: null });
    closeAuth();
    updateNavbar();
...
async function handleRegister(e) {
  ...
  if (!name || !email || !pass || !conf) { alert('⚠️ Vui lòng điền đầy đủ thông tin.'); return; }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { alert('❌ Email không hợp lệ.'); return; }

  const pwCheck = isStrongPassword(pass);
  if (!pwCheck.valid) { alert(pwCheck.message); return; }
  if (pass !== conf)  { alert('❌ Xác nhận mật khẩu không khớp.'); return; }

  try {
    const cred = await createUserWithEmailAndPassword(auth, email, pass);
    saveCurrentUser({ name, email, avatar: null });
    closeAuth();
    updateNavbar();
...
function togglePassword(inputId, btn) {
  ...
  if (input.type === "password") {
    input.type = "text";
    btn.innerHTML = `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="eye-svg"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="1" x2="23" y2="23"></line></svg>`;
  } else {
    input.type = "password";
    btn.innerHTML = `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="eye-svg"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>`;
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  updateNavbar();
  // Highlight tab đang active trên navbar
  const tenFile = location.pathname.split('/').pop();
  document.querySelectorAll('nav ul li a').forEach(a => {
    if (a.getAttribute('href') === tenFile) a.classList.add('trang_hien_tai');
  });
});
...
function checkout() {
  if (da_chon.size === 0) { alert('Vui lòng chọn ít nhất 1 ghế!'); return; }
  if (!isLoggedIn()) {
    closeModal();
    openAuth('register');
...
function closePay() {
  const el = document.getElementById('hop_thoai_thanh_toan_id');
...
async function processPayment(btn) {
  ...
  try {
    ...
    console.log("🎉 Đã lưu vé thành công! Mã:", code);
  } catch (err) {
...
window.checkout = function() {
  ...
  setTimeout(() => {
    ...
    if (posterEl) { posterEl.src = currentMoviePoster; posterEl.alt = currentMovieTitle; posterEl.style.display = 'block'; }
    const nameEl    = document.getElementById('payMovieName');    if (nameEl) nameEl.textContent = currentMovieTitle;
    const nameRowEl = document.getElementById('payMovieNameRow'); if (nameRowEl) nameRowEl.textContent = currentMovieTitle;
    const tenPhimVeEl = document.getElementById('ten_phim_ve_id'); if (tenPhimVeEl) tenPhimVeEl.textContent = currentMovieTitle;
    const bannerEl  = document.getElementById('newCustomerBanner'); if (bannerEl) bannerEl.style.display = isNewCustomer ? 'flex' : 'none';
    syncPayTotal();
  }, 30);
};
...
function recalcTotal() {
  ...
  if (isNewCustomer) el.style.color = '#22c55e';
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function moveTooltip(e) {
    ...
    let left = (window.innerWidth - e.clientX >= tw + OFFSET) ? e.clientX + OFFSET : e.clientX - tw - OFFSET;
  ...
  document.querySelectorAll('.the_phim[data-info-title]').forEach(card => {
    card.addEventListener('mouseenter', function(e) { clearTimeout(hideTimer); fillTooltip(this); moveTooltip(e); tooltip.classList.add('visible'); });
    card.addEventListener('mousemove',  moveTooltip);
    card.addEventListener('mouseleave', () => { hideTimer = setTimeout(() => tooltip.classList.remove('visible'), 80); });
  });
...
function batTatMenuRap(e) {
  e.stopPropagation();
  const li = document.querySelector('.co_menu_con');
  if (li) li.classList.toggle('dang_mo');
}

function locRap(tuKhoa) {
  const thanhPhoChon = document.getElementById('loc_thanh_pho_id').value;
  const kw = tuKhoa.toLowerCase().trim();
  document.querySelectorAll('.muc_rap').forEach(function(muc) {
    const tenRap   = muc.querySelector('.ten_rap').textContent.toLowerCase();
    const diaChiRap= muc.querySelector('.dia_chi_rap').textContent.toLowerCase();
    const khopTP   = thanhPhoChon === '' || muc.getAttribute('data-thanh-pho') === thanhPhoChon;
    const khopKW   = kw === '' || tenRap.includes(kw) || diaChiRap.includes(kw);
    muc.classList.toggle('an_rap', !(khopTP && khopKW));
  });
}

document.addEventListener('click', function(e) {
  const li = document.querySelector('.co_menu_con');
  if (li && !li.contains(e.target)) li.classList.remove('dang_mo');
});

window.addEventListener('DOMContentLoaded', function() {
  const selectTP = document.getElementById('loc_thanh_pho_id');
  if (selectTP) selectTP.addEventListener('change', function() { locRap(document.getElementById('o_tim_rap_id').value); });
});

function chuyenTrangRap(tenRap) {
  const path = window.location.pathname;
  let prefix;
  if      (path.includes('/Danhsachrap/'))  prefix = '';
  else if (path.includes('/thongtin/'))     prefix = '../Danhsachrap/';
...
// ════════════════════════════════════════════════════════════
//  GẮN VÀO WINDOW (gọi từ HTML onclick)
// ════════════════════════════════════════════════════════════
...
window.syncPayTotal   = syncPayTotal;